...
def _maybe_int(s: str):
    try:
        return int(str(s), 0)  # supports hex like "0x00"
    except Exception:
        return s

def _load_ini(path: str = "config.ini"):
...
def _merge_ini_into_args(args, defaults: dict):
    """
    Merge values from [dict_trainer] section into argparse args.
    CLI arguments take precedence; INI only overrides fields still at their default.
    Missing keys keep the original defaults.
    """
    cfg = _load_ini()
    if not cfg or not cfg.has_section("dict_trainer"):
        return args
    dt = cfg["dict_trainer"]
    keys = [
        "pth","dict_size","line_bits","pad_byte","lowercase","out_dir","algo",
        "min_match","max_match","window","overhead",
        "k_min","k_max","k_step","sample_step",
        "chunk","step","k","d","threads","dict_id",
    ]
    for k in keys:
        attr = k.replace("-", "_")
        ...
        if k not in dt:
            continue
        # Only override if user didn't set via CLI (args currently equals default)
        if getattr(args, attr, None) != defaults.get(attr, None):
            continue
        val = dt.get(k)
        if attr in ("lowercase",):
            val = _bool_from_str(val)
        elif attr in ("pad_byte","dict_size","line_bits","min_match","max_match",
                      "window","overhead","k_min","k_max","k_step","sample_step",
                      "chunk","step","k","d","threads","dict_id"):
            val = _maybe_int(val)
...
def main():
    ...
    # Merge optional config.ini values (if present). CLI args take precedence.
    _defaults = {
        'pth':'data_set/yolov8s_weights.pth','dict_size':4096,'line_bits':512,'pad_byte':0x00,'lowercase':False,
        'out_dir':'dicts_out','algo':'zstdlib','min_match':3,'max_match':64,'window':32768,'overhead':3,
        'k_min':16,'k_max':64,'k_step':8,'sample_step':3,'chunk':131072,'step':65536,'k':0,'d':0,'threads':-1,'dict_id':0,
    }
    args = _merge_ini_into_args(args, _defaults)
